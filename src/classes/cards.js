...
class Content {
    ...
    sortContent(content) {
        if (content) {
            let sortedContent = content.sort((a, b) => a.name.localeCompare(b.name));
            return this.removeDuplicates(sortedContent, 'name');
        }
    }
    ...
    updateURL(filterName) {
        ...
        history.pushState(null, `${filterName} | TW Delivery`, urlParam);
    }
